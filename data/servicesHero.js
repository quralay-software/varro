export const servicesHeroData = {
    ru: {
        company: 'Varro Operating Group',
        title: 'Современные и инновационные решения в нефтегазовой отрасли',
        subtitle: 'Комплексные решения для вашего бизнеса',
        description: 'Полный комплекс услуг по добыче, переработке и поставке нефтегазовых ресурсов с использованием передовых технологий и соблюдением экологических стандартов.',
        button: 'Подробнее о наших услугах',
        image: {
            src: '/images/img1.jpg',
            alt: 'Нефтегазовая промышленность'
        },
        stats: {
            experience: {
                value: '15+',
                label: 'Лет опыта'
            },
            projects: {
                value: '100+',
                label: 'Проектов'
            },
            clients: {
                value: '50+',
                label: 'Клиентов'
            }
        }
    },
    kk: {
        company: 'Varro Operating Group',
        title: 'Мұнай-газ саласындағы заманауи және инновациялық шешімдер',
        subtitle: 'Сіздің бизнесіңіз үшін кешенді шешімдер',
        description: 'Озық технологияларды қолдана отырып және экологиялық стандарттарды сақтай отырып, мұнай-газ ресурстарын өндіру, өңдеу және жеткізу бойынша толық қызметтер кешені.',
        button: 'Біздің қызметтер туралы толығырақ',
        image: {
            src: '/images/img1.jpg',
            alt: 'Мұнай-газ өнеркәсібі'
        },
        stats: {
            experience: {
                value: '15+',
                label: 'Жыл тәжірибе'
            },
            projects: {
                value: '100+',
                label: 'Жобалар'
            },
            clients: {
                value: '50+',
                label: 'Клиенттер'
            }
        }
    },
    en: {
        company: 'Varro Operating Group',
        title: 'Modern and Innovative Solutions in Oil and Gas Industry',
        subtitle: 'Comprehensive Solutions for Your Business',
        description: 'A full range of services for the extraction, processing, and supply of oil and gas resources using advanced technologies and compliance with environmental standards.',
        button: 'Learn More About Our Services',
        image: {
            src: '/images/img1.jpg',
            alt: 'Oil and Gas Industry'
        },
        stats: {
            experience: {
                value: '15+',
                label: 'Years Experience'
            },
            projects: {
                value: '100+',
                label: 'Projects'
            },
            clients: {
                value: '50+',
                label: 'Clients'
            }
        }
    }
};
